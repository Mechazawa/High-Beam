...
| Plugin runtime | rquickjs 0.12 (QuickJS binding) + llrt module crates | ~1 MB embed, per-context memory caps, interrupt hooks for timeouts; the llrt crates supply the Node/Web API surface (`fetch`, `node:path`, `node:fs`, `URL`, web-streams, …) on top of bare QuickJS |
