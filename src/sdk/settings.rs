...
/// Merge a plugin's manifest defaults with the user's stored overrides.
/// User-set values win; otherwise the manifest default applies. Unknown user
/// keys are dropped — a plugin author renaming an option shouldn't leave
/// stale stuff floating in the runtime.
#[must_use]
pub fn merge_options<S: BuildHasher>(
    defs: &[crate::plugins::manifest::OptionDef],
...
mod tests {
    use super::*;
    use crate::plugins::manifest::{OptionDef, OptionKind};

    fn def_string(key: &str, default: &str) -> OptionDef {
        OptionDef {
            key: key.into(),
            label: key.into(),
    ...
    fn def_int(key: &str, default: i64) -> OptionDef {
        OptionDef {
            key: key.into(),
            label: key.into(),
            kind: OptionKind::Int {
    ...
    #[test]
    fn merge_uses_user_value_when_set() {
    ...
    #[test]
    fn merge_falls_back_to_manifest_default() {
        let defs = vec![def_string("user", "alice"), def_int("limit", 10)];
        let user = HashMap::new();
        let merged = merge_options(&defs, &user);
        assert_eq!(merged.get("user"), Some(&JsonValue::String("alice".into())));
        assert_eq!(merged.get("limit"), Some(&JsonValue::Number(10.into())));
    }
